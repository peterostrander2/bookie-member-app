 ...
 * Uses cache: 'no-store' to prevent stale data from browser/SW cache.
 */
export const authFetch = async (url) => {
  ...
  const response = await apiFetch(url, { headers, cache: 'no-store' });
  if (!response.ok) {
...
/**
 * Safely parse JSON from response. Returns null on failure instead of throwing.
 */
 ...
 * Note: authFetch already uses cache: 'no-store'
 */
